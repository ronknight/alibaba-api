...
# Log file names
log_file = f"{LOG_DIR}product_schema_add_draft_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}product_schema_add_draft_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

try:
    # Make the API request
    add_product_schema_draft_request = requests.get(url, params=params)
    add_product_schema_draft_response = add_product_schema_draft_request.json()

    # Log API request
    with open(log_file, "w") as f:
        json.dump({
            "request_params": remove_sensitive_info(params),
            "response": add_product_schema_draft_response,
        }, f, indent=4)

    # Example of handling the response data
    if add_product_schema_draft_response.get("error_response"):
        with open(error_log_file, "w") as f:
            json.dump({
                "request_params": remove_sensitive_info(params),
                "response": add_product_schema_draft_response,
            }, f, indent=4)
        print(f"Error: {add_product_schema_draft_response['error_response']['msg']}")
    else:

...
# Log file names
log_file = f"{LOG_DIR}categoryget_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}categoryget_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

try:
    # Make the API request
    get_category_request = requests.get(url, params=params)
    get_category_response = get_category_request.json()
    ...
    if get_category_response.get("error_response"):
        with open(error_log_file, 'w') as f:
            json.dump({
                "request_params": remove_sensitive_info(params),
                "response": get_category_response,
            }, f, indent=4)
        print(f"Error: {get_category_response['error_response']['msg']}")
    else:

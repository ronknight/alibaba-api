...
# Log file names
log_file = f"{LOG_DIR}product_schema_render_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}product_schema_render_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

try:
    # Make the API request
    render_product_schema_request = requests.get(url, params=params)
    render_product_schema_response = render_product_schema_request.json()

    # Log API request
    with open(log_file, "w") as f:
        json.dump({
            "request_params": remove_sensitive_info(params),
            "response": render_product_schema_response,
        }, f, indent=4)

    # Example of handling the response data
    if render_product_schema_response.get("error_response"):
        with open(error_log_file, "w") as f:
            json.dump({
                "request_params": remove_sensitive_info(params),
                "response": render_product_schema_response,
            }, f, indent=4)
        print(f"Error: {render_product_schema_response['error_response']['msg']}")
    else:

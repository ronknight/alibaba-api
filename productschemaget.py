...
# Log file names
log_file = f"{LOG_DIR}product_schema_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}product_schema_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

try:
    # Make the API request
    get_product_schema_request = requests.get(url, params=params)
    get_product_schema_response = get_product_schema_request.json()

    # Log API request
    with open(log_file, "w") as f:
        json.dump({
            "request_params": remove_sensitive_info(params),
            "response": get_product_schema_response,
        }, f, indent=4)

    # Example of handling the response data
    if get_product_schema_response.get("error_response"):
        with open(error_log_file, "w") as f:
            json.dump({
                "request_params": remove_sensitive_info(params),
                "response": get_product_schema_response,
            }, f, indent=4)
        print(f"Error: {get_product_schema_response['error_response']['msg']}")
    else:

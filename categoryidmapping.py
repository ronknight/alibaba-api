...
# Log file names
log_file = f"{LOG_DIR}category_id_mapping_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}category_id_mapping_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

try:
    # Make the API request
    category_id_mapping_request = requests.get(url, params=params)
    category_id_mapping_response = category_id_mapping_request.json()

    # Log API request
    with open(log_file, "w") as f:
        json.dump({
            "request_params": remove_sensitive_info(params),
            "response": category_id_mapping_response,
        }, f, indent=4)

    # Example of handling the response data
    if category_id_mapping_response.get("error_response"):
        with open(error_log_file, "w") as f:
            json.dump({
                "request_params": remove_sensitive_info(params),
                "response": category_id_mapping_response,
            }, f, indent=4)
        print(f"Error: {category_id_mapping_response['error_response']['msg']}")
    else:

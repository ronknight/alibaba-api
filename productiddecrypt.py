...
params = {
    ...
    "v": "2.0",
    "language": "ENGLISH",  # Language parameter set to ENGLISH
...
# Log file names
log_file = f"{LOG_DIR}productdecrypt_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}productdecrypt_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

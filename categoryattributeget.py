...
# Check if cat_id is provided as command-line argument
if len(sys.argv) < 2:
...
params = {
    ...
    "v": "2.0",
    "cat_id": cat_id,  # Category ID from command-line argument
}
...
# Log file names
log_file = f"{LOG_DIR}categoryattributeget_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}categoryattributeget_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

try:
    # Make the API request
    get_category_attribute_request = requests.get(url, params=params)
    get_category_attribute_response = get_category_attribute_request.json()

    # Log API request
    with open(log_file, 'w') as f:
        json.dump({
            "request_params": remove_sensitive_info(params),
            "response": get_category_attribute_response,
        }, f, indent=4)

    # Example of handling the response data
    if get_category_attribute_response.get("error_response"):
        with open(error_log_file, 'w') as f:
            json.dump({
                "request_params": remove_sensitive_info(params),
                "response": get_category_attribute_response,
            }, f, indent=4)
        print(f"Error: {get_category_attribute_response['error_response']['msg']}")
    else:

...
if len(sys.argv) < 2:
    print("Usage: python script.py <group_id>")
    sys.exit(1)

group_id = sys.argv[1]  # Get group_id from command-line argument
...
params = {
    ...
    "v": "2.0",
    "group_id": group_id,  # Group ID from command-line argument
}
...
# Log file names
log_file = f"{LOG_DIR}productgroupget_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}productgroupget_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

try:
    # Make the API request
    get_group_request = requests.get(url, params=params)
    get_group_response = get_group_request.json()

    # Log API request
    with open(log_file, 'w') as f:
        json.dump({
            "request_params": remove_sensitive_info(params),
            "response": get_group_response,
        }, f, indent=4)

    # Example of handling the response data
    if get_group_response.get("error_response"):
        with open(error_log_file, 'w') as f:
            json.dump({
                "request_params": remove_sensitive_info(params),
                "response": get_group_response,
            }, f, indent=4)
        print(f"Error: {get_group_response['error_response']['msg']}")
    else:
